...
def build_ubl_invoice(data: InvoiceRequest) -> bytes:
    ...
    due = data.due_date or (
        date.fromisoformat(data.issue_date) + timedelta(days=30)
    ).isoformat()
    ...
    _add(
        supplier_party,
        f"{{{NS_CBC}}}EndpointID",
        data.seller.endpoint_id,
        {"schemeID": data.seller.endpoint_scheme},
    )
    ...
    _add(
        s_legal,
        f"{{{NS_CBC}}}CompanyID",
        data.seller.legal_id,
        {"schemeID": data.seller.legal_id_scheme},
    )
    ...
    _add(
        customer_party,
        f"{{{NS_CBC}}}EndpointID",
        data.buyer.endpoint_id,
        {"schemeID": data.buyer.endpoint_scheme},
    )
    ...
    _add(
        monetary,
        f"{{{NS_CBC}}}TaxExclusiveAmount",
        f"{total_ht:.2f}",
        {"currencyID": data.currency},
    )
    _add(
        monetary,
        f"{{{NS_CBC}}}TaxInclusiveAmount",
        f"{total_ttc:.2f}",
        {"currencyID": data.currency},
    )
    _add(
        monetary,
        f"{{{NS_CBC}}}PayableAmount",
        f"{total_ttc:.2f}",
        {"currencyID": data.currency},
    )
